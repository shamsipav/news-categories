...
<Modal bind:this={ modal } hasFooter={ false } title="Редактирование новости">
    <Form method="PUT" action="https://localhost:7220/api/News/{news.id}" content="application/json" reset={ false } on:success={ handleSuccess }>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <textarea type="text" name="text" class="form-control" id="text" cols="30" rows="5">{news.text}</textarea>
        </div>
        <div class="mb-3">
                    ...
                    {#each categories as category}
                      <div class="form-check">
                            ...
                            {#if newsCategoriesIds.includes(category.id)}
                                <input class="form-check-input" type="checkbox" name="categories" value="{category.id}" id="checkChecked{category.id}" checked>
...
<Modal bind:this={ modalDelete } hasFooter={ false } title="Удаление новости">
    <Form method="DELETE" action="https://localhost:7220/api/News/{news.id}" content="application/json" redirect="/news">
